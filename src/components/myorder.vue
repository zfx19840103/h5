          ...
          <li v-for="(item, index) in myorderData.list" :key="index">
            <div @click="orderdetail(item)">
              ...
              <div class="top">
                ...
                <em v-bind:class="{ 'col': item.pay_status == 0 }">{{order_status_func(item.order_status)}}</em>
              </div>
              <div class="center">
            ...
            <div class="bottom">
              ...
              <div v-if="item.usage_scenario == 'bytemoon_pay'">
                ...
                <button class="linkDetail invoiceopen" v-if="(item.order_status == 5 || item.order_status == 4) && item.invoice_status == 0"
                  @click="invoiceopen(item)">申请开票</button>

                <button v-if="(item.order_status == 5 || item.order_status == 4) && (item.invoice_status == 1 || item.invoice_status == 2 || item.invoice_status == 4)"
                  @click="invoiceshow(item)">查看发票</button>
              </div>
      ...
      return {
        ...
        myorderData: {
          ...
          list1: [{
              ...
              "user_id": -27383510.62338373,
              "snapshoot_cnt": {
                "out_biz_code": "",
                "sku_list": [{
                  "itemCode": "sit reprehenderit",
                  "shopPrice": -84546738.01328765,
                  "itemName": "xxxxxx月饼礼盒",
                  "images": [
              ...
              "order_status": 2,
              bytemoon_pay_type: 2,
              order_code: '453452352',
              "created_at": "2020-07-16 10:25:23",
            },
            {
